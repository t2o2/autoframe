...
1. Linear GraphQL `get_issue` (bash+curl via `linear_gql`), Linear GraphQL `workflowStates` query (bash+curl), and Linear GraphQL `comments` query (bash+curl) — in parallel
...
Call `linear_gql createAttachment`:
...
Call Linear attachment upload (bash+curl, see ticket-process template):
...
Call Linear GraphQL `get_issue` (bash+curl via `linear_gql`) with the issue UUID (from the issue fetched in Phase 0) and confirm `attachments` is non-empty. The count must equal the number of files you uploaded.

If `attachments` is empty or the count is lower than expected:

- Do **not** proceed to Phase 6
- Re-call Linear attachment upload (bash+curl, see ticket-process template) for any missing files
...
Build the review comment. Post via Linear GraphQL `commentCreate` mutation (bash+curl). Every section is **mandatory**.
...
# linear_gql issueUpdate → { id, statusId: <human_review_id>, assignee: "<GIT_EMAIL>" }
```
...
# linear_gql issueUpdate → { id, statusId: <changes_required_id>, assignee: "<GIT_EMAIL>" }
```
...
```
Post a Linear comment and proceed with the conservative default if non-interactive, or raise via ask_user_question:
...
Phase 5: Visual Proof [MANDATORY — no skip]
  ...
  │ Empty proof dir = FAIL — ask_user_question         │
  └──────────────────────┬───────────────────────────┘
                         │
                         ▼
...
Phase 7: Update Linear Status
  save_issue: Human Review   save_issue: Changes Required
               ...
               ▼                   ▼
Phase 8: Hand Off
  Post a Linear comment and proceed with the conservative default if non-interactive, or raise via ask_user_question:           Inform user: FAILED
