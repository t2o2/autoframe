...
1. Call Linear GraphQL `get_issue` (bash+curl via `linear_gql`) with identifier `$ARGUMENTS`
2. If the response includes a `parentId`, call Linear GraphQL `get_issue` (bash+curl via `linear_gql`) with that `parentId` to get the parent's `identifier` (e.g. `TICKET-24`)
...
1. Linear GraphQL `workflowStates` query (bash+curl) — find the "Done" status ID
...
3. Linear GraphQL `issueUpdate` mutation (bash+curl) → `{ id: "<ticket-id>", statusId: "<done_id>", assignee: "<GIT_EMAIL>" }`
4. Linear GraphQL `commentCreate` mutation (bash+curl):
