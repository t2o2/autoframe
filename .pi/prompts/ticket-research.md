...
1. Linear GraphQL `get_issue` (bash+curl via `linear_gql`) — full ticket details
2. Linear GraphQL `workflowStates` query (bash+curl) — valid status IDs for the team
3. Linear GraphQL `comments` query (bash+curl) — any prior discussion or previous attempt notes
...
```
linear_gql issueUpdate → { id, statusId: <research_id> }
```
...
Use the returned text as the clarification context. If the script exits 2 (no credentials), fall back to `ask_user_question` tool (or post a Linear comment and proceed with the conservative default if non-interactive).
...
1. Post the full research document as a ticket comment:
   ...
   Linear GraphQL `commentCreate` mutation (bash+curl) → `{ issue: "$ARGUMENTS", body: <research document> }`

2. Set ticket status to `Research Pending Approval`:

   Linear GraphQL `issueUpdate` mutation (bash+curl) → `{ id, statusId: <pending_research_approval_id> }`
