...
fetch_pending_tickets() {
    ...
    response=$(curl -sf \
        ...
        -d "{\"query\":\"{ issues(filter:{team:{key:{eq:\\\"${LINEAR_TEAM_KEY}\\\"}},state:{name:{in:[\\\"Research Approved\\\"]}}}) { nodes { identifier } } }\"}" \
...
main() {
    ...
    while true; do
        ...
        revert_stale_linear_claims "Planning" "Research Approved" "plan"
        cycle=$((cycle + 1))
