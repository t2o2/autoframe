...
3. Linear GraphQL `comments` query (bash+curl) — find the research comment from the research agent
...
```
linear_gql issueUpdate → { id, statusId: <planning_id> }
```
...
1. Post the full implementation plan as a ticket comment:
   ...
   Linear GraphQL `commentCreate` mutation (bash+curl) → `{ issue: "$ARGUMENTS", body: <plan> }`

2. Set ticket status to `Plan Pending Approval`:

   Linear GraphQL `issueUpdate` mutation (bash+curl) → `{ id, statusId: <pending_plan_approval_id> }`
